			...
			if($fechaLlegadaViaje <= $fecha_act AND $estado == 2){
        		...
        		$conect7->execute();
       	 		...
       	 		foreach($rs as $row3){
						...
						':parapagar'=>$parapagar,
						':precio'=>$precioDividido));
                        // inserto las puntuaciones
                        $puntuar='1';
                        $conect7= $conexion->prepare('INSERT INTO puntajes(idpuntuado,idpuntuador,idviaje,puntuar) VALUES(:piloto,:idpuntuador,:idviaje,:puntuar)');
						$conect7->execute(array(
						':piloto'=> $usuario,
						':idpuntuador'=> $idpostulante,
						':idviaje'=> $id,
						':puntuar'=>$puntuar));
					}
       	 		}
...
function traer_comentarios($id){
    
    
    $con = mysqli_connect("localhost", "root", "", "aventon") or die ("no se pudo conectar");
        mysqli_select_db($con, 'aventon');
        $query = "SELECT * FROM comentarios WHERE idviaje = '$id'";
        $resultado = mysqli_query($con, $query);
        $rs = mysqli_fetch_array($resultado);
...
function traer_respuestas($id){
    
    
    $con = mysqli_connect("localhost", "root", "", "aventon") or die ("no se pudo conectar");
        mysqli_select_db($con, 'aventon');
        $query = "SELECT * FROM respuestas WHERE idcomentario = '$id'";
        $resultado = mysqli_query($con, $query);
        $rs = mysqli_fetch_array($resultado);
